...
[Unit]
Description=Live storage service
...
echo "Data is preserved under: ${INSTALL_DIR}/data"
echo "API key file: ${INSTALL_DIR}/data/api_key"
echo "Status:"
systemctl --no-pager --full status "live-${APP_NAME}.service" || true
echo
echo "Check local health:"
echo "curl -H \"X-API-Key: \$(cat ${INSTALL_DIR}/data/api_key)\" http://127.0.0.1:19280/api/health"
