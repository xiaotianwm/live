...
REPO_NAME="live"
BRANCH="main"
PACKAGE_URL="https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/${BRANCH}/storage/storage-ubuntu-amd64.zip"
...
ZIP_PATH="${TMP_DIR}/storage-ubuntu-amd64.zip"
WORK_DIR="${TMP_DIR}/storage"

echo "Downloading storage package..."
download "${PACKAGE_URL}" "${ZIP_PATH}"

mkdir -p "${WORK_DIR}"
unzip -q "${ZIP_PATH}" -d "${WORK_DIR}"

chmod +x "${WORK_DIR}/storage" "${WORK_DIR}/install.sh" "${WORK_DIR}/start.sh" "${WORK_DIR}/stop.sh"
...
echo "Service status:"
echo "  systemctl status live-storage.service"
echo "Logs:"
echo "  journalctl -u live-storage.service -f"
